...
public class PersonaAuthenticationFilter extends AuthenticationFilter {
    private final Logger logger = LogManager.getLogger(PersonaAuthenticationFilter.class);
    ...
    public PersonaAuthenticationFilter(AuthenticationManager authenticationManager) {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        try {
            if (postToAuthenticate(httpRequest, resourcePath)) {
                ...
                processUsernamePasswordAuthentication(httpRequest, httpResponse, username,
                        password);
                return;
            }
            ...
            if (tokenHeader.isPresent()) {
                logger.debug(
                        "Trying to authenticate user by authtoken(in Header) method. Token: {}",
                        tokenHeader);
            ...
            logger.debug("PersonaAuthenticationFilter is passing request down the filter chain");
    ...
    private Authentication tryToAuthenticateWithToken(Optional<String> token,
        ...
        PreAuthenticatedAuthenticationToken requestAuthentication =
                new RequestPreAuthenticatedAuthenticationToken(
    ...
    private Authentication tryToAuthenticateWithCookie(HttpServletRequest httpRequest) {
        Authentication requestAuthentication =
                new RequestCookiePreAuthenticatedAuthenticationToken(null, null,
                        httpRequest);
        return tryToAuthenticate(requestAuthentication);
    }
