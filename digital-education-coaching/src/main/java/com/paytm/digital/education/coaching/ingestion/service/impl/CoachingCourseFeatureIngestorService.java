...
public class CoachingCourseFeatureIngestorService extends IngestorServiceHelper
    ...
    @Value("${coaching.producer.courseFeatureApi.url}")
    private String producerCourseFeatureApiUrl;
    ...
    public IngestorResponse ingest() {
        ...
        return this.processRecords(courseFeatureData,
                failedCourseFeatureFormList, CoachingCourseFeatureForm.class, TYPE
        );
    }
    ...
    protected <T> void upsertNewRecords(final T form, final List<Object> failedDataList,
        ...
        try {
            if (null == featureForm.getCourseFacilityId()) {
                response = this.makeCall(IngestorCoachingCourseFeatureTransformer.convert(
                        featureForm), HttpMethod.POST);
            } else {
                response = this.makeCall(IngestorCoachingCourseFeatureTransformer.convert(
                        featureForm), HttpMethod.PUT);
            }
    ...
    protected <T> void upsertFailedRecords(T form, Class<T> clazz) {
        ...
        try {
            if (null == featureForm.getInstituteId()) {
                this.makeCall(IngestorCoachingCourseFeatureTransformer.convert(
                        featureForm), HttpMethod.POST);
            } else {
                this.makeCall(IngestorCoachingCourseFeatureTransformer.convert(
                        featureForm), HttpMethod.PUT);
    ...
    private ResponseEntity<CoachingCourseFeatureDTO> makeCall(
            final CoachingCourseFeatureDataRequest request, final HttpMethod method)
        ...
        final HttpRequestDetails requestDetails = HttpRequestDetails.builder()
                ...
                .url(this.producerCourseFeatureApiUrl)
                .body(request)
                .headers(httpHeaders)
                .requestApiName("CoachingCourseFeatureIngestorService")

                .build();

        return this.httpUtil.exchange(this.restTemplateFactory.getRestTemplate(
                HttpConstants.GENERIC_HTTP_SERVICE), requestDetails, CoachingCourseFeatureDTO.class);
    }
}
